...
- [`settings.md`](settings.md) — every Settings section saves real, persisted mutations, and Tax/payment-methods/branch-scope/freeze/name+logo/referrer-reward genuinely drive behavior; but several settings are **saved-but-not-enforced**: cancellation policy (customer flow hardcoded 24h; the Settings engine is dead code), brand colors (never applied to CSS vars), the referral friend-reward + monthly cap, and the operator refund limit (setting doesn't exist).
