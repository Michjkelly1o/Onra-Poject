...
- [`customer-lead-lifecycle.md`](customer-lead-lifecycle.md) — the lifecycle **logic & flow** deep-dive: the three overlapping classifications (wallet segment vs AI lifecycle tag vs human follow-up status), the exact tag precedence (Churned > At Risk > Won-back > New Active > Loyal Active > Trialist > Lead) with each stage's condition, the follow-up pipeline + tasks/triggers/outcomes, and the lead → trialist → active → churned → won-back journey.
