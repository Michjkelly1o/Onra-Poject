// ─────────────────────────────────────────────────────────────────────────────
// Query engine — runs the agent's analysis requests against the catalog.
// The MODEL chooses dataset/metric/grouping/filters/chart; the SERVER computes,
// so numbers are always grounded (no hallucinated figures). Returns an InsightCard.
// ─────────────────────────────────────────────────────────────────────────────
import type { AuthContext } from "@/lib/agent/auth";
import { branchFilter } from "@/lib/data/scope";
import { AED, type InsightCard } from "@/lib/agent/cards";
import { CATALOG, type Dataset, type FieldMeta, type Row } from "@/lib/data/catalog";
...
export function runAnalyze(ctx: AuthContext, spec: AnalyzeSpec): InsightCard {
  ...
  const unit = spec.unit ?? (metricRow ? "count" : "count");
  ...
  if (spec.group_by) {
    ...
    let points = [...groups.entries()].map(([k, rs]) => ({
    ...
    // Data-viz rules: time → line; otherwise bar (never auto-pick a donut — pies
    // read poorly for magnitudes like revenue). Donut only when explicitly asked.
    if (viz === "auto") viz = isDate ? "line" : "bar";
    if (isDate && viz !== "table") viz = "line";
    // A donut only reads well as a few-slice share; fall back to bar otherwise,
    // and never use it for money amounts.
...
export function runList(ctx: AuthContext, spec: ListSpec): InsightCard {
  const ds = CATALOG[spec.dataset];
  if (!ds) return emptyCard(`Unknown dataset "${spec.dataset}".`);
  let rows = applyScopeFilterDate(ctx, ds, spec);
  if (rows.length === 0) return emptyCard("No records match that query.");
