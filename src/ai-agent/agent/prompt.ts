...
3. STEP 3 · Review & mapping: call \`propose_mapping({ entity })\`. The editable mapping card is shown. Ask the user to review/accept before moving on.
   ...
   • If \`inspect_source\` returned \`status: "n_a"\` — the entity has NO branch dimension (tax_rates, class_categories, agreements, promo_codes, campaigns, pay_rates, staff, services, rooms, gift_cards, and every customer-history entity). Do NOT ask the user which branch. Do NOT mention branches at all. IMMEDIATELY call \`propose_mapping\` in the SAME assistant turn — the branch step is skipped entirely because these records are applied to branches later on the admin side (e.g. Apply tax rates), not at import time.
