    ...
    const content = (
        <>
            <span className="flex min-w-0 flex-1 items-center gap-1">
                ...
                <span className="min-w-0 flex-1 truncate text-base font-normal leading-6 text-[var(--colors-text-quaternary)]">{branchName}</span>
            </span>
            {canSwitch && <ChevronDown className="size-4 shrink-0 text-[var(--colors-text-quaternary)]" aria-hidden />}
        </>
    );
    const base = "flex w-full items-center gap-2 rounded-full border border-[var(--colors-border-secondary)] bg-white px-3 py-2 text-left";
