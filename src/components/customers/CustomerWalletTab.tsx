...
// with the ledger below. Reuses the Payments-tab metric-card + table chrome
// so it scans identically to the rest of the customer detail.
...
export function CustomerWalletTab({ customerId }: { customerId: string }) {
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto scrollbar-hide px-6 pt-5 pb-6 flex flex-col gap-6">
                {/* Balance + totals */}
                ...
                <div className="flex flex-col gap-3">
                    <p className="text-[16px] font-semibold text-[#101828]">Transaction history</p>
                    {txns.length === 0 ? (
                        <div className="border-1 border-dashed border-[#e4e7ec] rounded-[16px] py-10 flex flex-col items-center gap-1">
                            <p className="text-[14px] font-medium text-[#344054]">No wallet activity</p>
                            <p className="text-[13px] text-[#667085]">This customer has no account credit yet.</p>
                        </div>
                    ) : (
                        <>
                            <div className="border-1 border-[#e4e7ec] rounded-[16px] overflow-hidden">
                                <table className="w-full border-collapse">
                                    ...
                                    <tbody>
                                        {paged.map(t => (
                                            <tr key={t.id} className="hover:bg-[#f9fafb]/50 transition-colors">
                                                <td className={TD}>
                                                    {t.type === "credit" ? (
