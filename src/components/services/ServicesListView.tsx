...
const ALL_STATUSES: ServiceStatus[] = ["active", "inactive"];
...
// Shared list surface for the Services module. Rendered by three thin page
// wrappers: the umbrella /admin/services (all types) plus the dedicated
...
export function ServicesPageInner({ fixedType }: { fixedType?: "private" | "recovery" }) {
    ...
    // comes from `fixedType`; the umbrella /admin/services route falls back to
    // the legacy ?type= deep-link. Drives the list scope + contextual title.
    const searchType = useSearchParams().get("type");
    const typeScope = fixedType ?? searchType;
    ...
    const filteredRows = useMemo(() => {
        ...
        return allRows.filter(r => {
            ...
            if (filtersEnabled) {
                // Status pill filters only the active table; archived services are
                // exempt — they always render in the Archived section (policy §3).
                if (r.status !== "archived" && applied.statuses.length && !applied.statuses.includes(r.status)) return false;
    ...
    // Archived services leave the active table → the shared Archived section.
    ...
    function toggleAllRows(rows: ServiceRow[], check: boolean) {
        const next = new Set(selectedIds);
    ...
    // ─── Bulk derived flags ────────────────────────────────────────────────
    const selectedRows = useMemo(() => [...activeRows, ...archivedRows].filter(r => selectedIds.has(r.id)), [activeRows, archivedRows, selectedIds]);
    ...
    const listPath = fixedType
        ? `/admin/products/${fixedType}`
        : (typeScope ? `/admin/services?type=${typeScope}` : "/admin/services");
    function handleAdd() {
