            ...
            <div className="relative bg-white rounded-[16px] w-[600px] max-w-[90vw] shadow-[0px_20px_24px_-4px_rgba(16,24,40,0.08),0px_8px_8px_-4px_rgba(16,24,40,0.03)] flex flex-col overflow-hidden">
                <div className="pt-6 px-6 pb-5 border-b border-[var(--colors-border-secondary)] relative">
                    {agreementName ? (
                        <div className="flex flex-col gap-1 pr-10">
                            <h3 className="font-semibold text-[18px] leading-[28px] text-[var(--colors-text-primary)] truncate">{agreementName}</h3>
                            <p className="text-[14px] text-[var(--colors-text-tertiary)]">
                                {versionLabel ? `${versionLabel} · ` : ""}Agreement content
                            </p>
                        </div>
                    ) : (
                        <h3 className="font-semibold text-[18px] leading-[28px] text-[var(--colors-text-primary)]">Agreement content</h3>
                    )}
                ...
                <div className="px-6 py-5">
                    {/* Single bordered panel — every content type renders as
                        ...
                        download link. */}
                    <div className="border-1 border-[var(--colors-border-secondary)] rounded-[12px] h-[360px] overflow-hidden bg-white">
                        <div
                            className="text-[16px] text-[var(--colors-text-primary)] leading-6 prose prose-headings:text-[var(--colors-text-primary)] prose-strong:text-[var(--colors-text-primary)] max-w-none h-full overflow-y-auto p-4"
