...
// ─────────────────────────────────────────────────────────────────────────────
// Onra Studio — Event detail page (/events/[id])
// ─────────────────────────────────────────────────────────────────────────────
//
// Full-screen detail page — same shell as the campaign detail (/marketing/[id]):
//   • 72px top header (X close + page title)
//   • DetailPageShell two-column frame
//   • 320px left sidebar (banner + key info + actions)
//   • Right panel: single scrollable body — Event configuration + Visibility
//     settings
//
// Events are a single-type marketing module, so there's no "type" row. They
// carry the full CTA set (Book an event / Buy a ticket / External link), so
// the booked-event, ticket-price and external-link rows all appear.
//
...
import {
    ...
    CursorBox, Calendar, Ticket01, Link01, CheckVerified02,
...
const MODAL_CONFIG: Record<ModalAction, {
    ...
    archive: {
        ...
        description: "It will be hidden from the events list and removed from the customer feed. You can recover archived events at any time.",
...
function EventSidebarBanner({ vm }: { vm: EventDetailVM }) {
...
function LeftSidebar({ vm, onAction, branches }: {
    ...
    const actions = (() => {
        if (vm.status === "archived") {
            return (
                <ActionBtn icon={<RefreshCcw01 className="w-5 h-5" />} label="Recover event" onClick={() => onAction("recover")} />
            );
        }
        if (vm.status === "inactive") {
            return (
                <>
                    <ActionBtn icon={<RefreshCcw01 className="w-5 h-5" />} label="Reactivate event" onClick={() => onAction("reactivate")} />
                    <ActionBtn icon={<Archive className="w-5 h-5" />} label="Archive event" onClick={() => onAction("archive")} />
                </>
            );
        }
        return (
            <>
                <ActionBtn icon={<Edit02 className="w-5 h-5" />} label="Edit event" onClick={() => onAction("edit")} />
                <ActionBtn icon={<Archive className="w-5 h-5" />} label="Archive event" onClick={() => onAction("archive")} />
                {canDelete ? (
                    <ActionBtn icon={<Trash01 className="w-5 h-5" />} label="Delete event" danger onClick={() => onAction("delete")} />
                ) : (
                    <ActionBtn icon={<SlashCircle01 className="w-5 h-5" />} label="Deactivate event" danger onClick={() => onAction("deactivate")} />
                )}
            </>
        );
    })();
    ...
    return (
        <div className="w-[320px] shrink-0 bg-white border border-[var(--colors-border-secondary)] rounded-[20px] flex flex-col overflow-hidden">
            <EventSidebarBanner vm={vm} />

            <div className="flex flex-col flex-1">
                <div className="flex flex-col gap-5 px-6 pt-5 pb-6 flex-1">
                    <div className="flex flex-col gap-1">
                        <h2 className="font-semibold text-[20px] leading-[30px] text-[var(--colors-text-primary)]">{vm.title}</h2>
                        {vm.description && (
                    ...
                    <div className="flex flex-col gap-3">
                        <SidebarField label="Event action" value={ACTION_LABEL[vm.actionType]} />
                        {vm.actionType === "book_event" && vm.ctaClassLabel && (
                            <SidebarField label="Booked event" value={vm.ctaClassLabel} />
                        )}
                ...
                <div className="px-6 pb-6 mt-auto">
                    ...
                    <p className="text-[14px] text-[var(--colors-text-quaternary)] mb-4">Event actions</p>
...
function RightPanel({ vm, branches }: { vm: EventDetailVM; branches: Branch[] }) {
    return (
        <div className="flex-1 min-w-0 flex flex-col overflow-hidden border border-[var(--colors-border-secondary)] rounded-[20px]">
            <div className="flex-1 overflow-y-auto scrollbar-hide px-6 py-6 flex flex-col gap-6">
                {/* ── Event configuration ── */}
                <SectionHeading>Event configuration</SectionHeading>
                <div className="grid grid-cols-2 gap-x-4 gap-y-5">
                    ...
                    {vm.actionType === "book_event" && vm.ctaClassLabel && (
                        <InlineStat icon={<CheckVerified02 className="w-4 h-4" />} label="Booked event" value={vm.ctaClassLabel} />
                    )}
                    ...
                    <InlineStat
                        ...
                        tooltip="Events can be shown on multiple branches"
                    />
                </div>
                ...
                <VisibilityCard
                    ...
                    subtitle={vm.multiLocation
                        ? "The event can be shown on multiple branches"
                        : "The event can be shown on a single branch"}
                ...
                <VisibilityCard
                    title="Packages"
                    subtitle="The event can be shown on multiple packages"
                ...
                <VisibilityCard
                    title="Customer"
                    subtitle="The event can be configured to target specific eligible users."
...
function EventDetailPageInner() {
    ...
    const returnTo = searchParams.get("returnTo") ?? LIST_PATH;
    ...
    if (!item) {
        return (
            <div className="h-screen bg-white flex flex-col items-center justify-center">
                <p className="text-[18px] font-semibold text-[var(--colors-text-primary)]">Event not found</p>
    ...
    function handleAction(a: "edit" | ModalAction) {
        if (a === "edit") {
            router.push(`/events/${id}/edit?returnTo=${encodeURIComponent(pathname)}`);
            return;
        }
        setConfirmAction(a);
    }
    ...
    function handleConfirm() {
        ...
        if (confirmAction === "archive") {
            ...
            showToast("Event deactivated", `${name} is no longer shown to members.`, "error", "slash");
            setConfirmAction(null);
        } else if (confirmAction === "recover") {
            updateMarketingItem(id, { status: "active" });
            showToast("Event recovered", `${name} has been recovered and is now active.`, "success", "refresh");
            setConfirmAction(null);
        } else if (confirmAction === "reactivate") {
            updateMarketingItem(id, { status: "active" });
            showToast("Event reactivated", `${name} is now active again.`, "success", "check");
            setConfirmAction(null);
        ...
        <div className="h-screen bg-white flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-6 h-[72px] shrink-0">
                ...
                <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                    <h1 className="font-semibold text-[20px] leading-[30px] text-[var(--colors-text-primary)]">Event details</h1>
                    <Breadcrumbs className="p-0 text-[12px]" />
                </div>
            </div>

            {/* Body — canonical DetailPageShell wraps the frame. */}
            <DetailPageShell
