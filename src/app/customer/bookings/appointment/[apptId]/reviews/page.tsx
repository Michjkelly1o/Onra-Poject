...
// ─────────────────────────────────────────────────────────────────────────────
// Customer — Appointment reviews (`/customer/bookings/appointment/[apptId]/reviews`)
// ─────────────────────────────────────────────────────────────────────────────
// The appointment mirror of the class "All reviews" page. Opened from "More
// reviews" on the appointment detail — the appointment's full review list (newest
// first), filtered by a topic chip and/or a star rating. Reads the SHARED
// appointmentRatings via the linked admin appointment.
...
import { useAppointmentReviews } from "@/lib/customer/bookings-data";
...
export default function AppointmentReviewsPage() {
    ...
    const reviews = useAppointmentReviews(booking?.adminAppointmentId);
    ...
    return (
        <div className="flex min-h-full flex-col">
            ...
            >
                ...
                <p className="min-w-0 flex-1 truncate text-center text-base font-semibold leading-6 text-[var(--brand-text)] font-heading">Ratings</p>
            ...
            {/* Star filter sheet — single-select, one star per row */}
            <CustomerSheet open={filterOpen} onClose={() => setFilterOpen(false)}>
                <div className="flex flex-col">
                    ...
                    <div className="flex items-center justify-between gap-3 pt-4">
                        <Button variant="secondary" size="xl" className="w-[106px] rounded-full" onClick={() => setDraftStar(null)}>
                            Reset
                        </Button>
                        <Button
                            variant="primary"
