...
// ─── Add-new split dropdown (Role / Staff) ─────────────────────────────────

function AddNewMenu({ onAddRole, onAddStaff }: { onAddRole: () => void; onAddStaff: () => void }) {
        ...
        <div ref={ref} className="relative">
            ...
            {open && (
                <div className="absolute right-0 top-[calc(100%+6px)] z-50 bg-white border-1 border-[#e4e7ec] rounded-[12px] shadow-[0px_12px_16px_-4px_rgba(16,24,40,0.08),0px_4px_6px_-2px_rgba(16,24,40,0.03)] py-1.5 min-w-[180px]">
                    <button type="button" onClick={() => { setOpen(false); onAddRole(); }}
                        ...
                        <UserSquare className="w-4 h-4 text-[#667085]" />Add role
                    </button>
                    <button type="button" onClick={() => { setOpen(false); onAddStaff(); }}
                        className="flex items-center gap-2.5 w-full px-4 py-[10px] text-[14px] font-medium text-[#344054] hover:bg-[#f9fafb] transition-colors">
...
export default function StaffPermissionsPage() {
    ...
    // ─── Toolbar handlers ─────────────────────────────────────────────────
    function handleAddRole()  { router.push("/staff/roles/new?returnTo=/admin/staff"); }
    function handleAddStaff() { router.push("/staff/members/new?returnTo=/admin/staff"); }
    ...
    function handleRoleAction(role: Role, kind: RoleRowActionKind) {
        if (kind === "view")             return router.push(`/staff/roles/${role.id}?returnTo=/admin/staff`);
        if (kind === "edit_details")     return router.push(`/staff/roles/${role.id}/edit?returnTo=/admin/staff`);
        if (kind === "edit_permissions") return router.push(`/staff/roles/${role.id}/permissions/edit?returnTo=/admin/staff`);
        if (kind === "add_staff")        return router.push(`/staff/members/new?roleId=${role.id}&returnTo=/admin/staff`);
        // Confirmable
    ...
    function handleStaffAction(s: Staff, kind: StaffRowActionKind) {
        if (kind === "view")          return router.push(`/staff/members/${s.id}?returnTo=/admin/staff`);
        if (kind === "edit_details")  return router.push(`/staff/members/${s.id}/edit?returnTo=/admin/staff`);
        if (kind === "change_role") {
    ...
    const totalNoun = tab === "roles"
        ...
        : (totalCount === 1 ? "staff" : "staffs");

    // ─── Active-filter dot for the Filter button ──────────────────────────
    const hasActiveFilter = tab === "roles"
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center gap-3">
                ...
                <AddNewMenu onAddRole={handleAddRole} onAddStaff={handleAddStaff} />
            </div>

            {/* View card — same h-[760px] chrome as /admin/products
                (memberships & packages) so the table area is fixed-height
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th className={cn(TH, "w-[100px]")}>
                                            <SortableHeader sortKey="staffs" currentSort={roleSortKey} dir={roleSortDir} onSort={toggleRoleSort}>Staffs</SortableHeader>
                                        </th>
