...
function BookedCard({ booking, customer, canPresent, onPresent, onViewDetails }: {
    ...
    return (
        ...
        >
            ...
            {/* Full-width Present button — BIGGER (h-12) for tablet/touch. Reuses
                the Schedule Present treatment (green CheckCircle + #067647). */}
            {active ? (
                // Active state (Figma 7962:41972) — green outline, opens the
                // Present confirmation modal.
                <Button
                    variant="secondary-gray"
                    size="xl"
                    className="w-full rounded-[10px] text-[16px] font-semibold text-[#067647] border-[#a9c3b4] hover:bg-[#ecfdf3] hover:text-[#067647]"
                ...
                // Disabled state (Figma 7962:42232) — muted gray check + gray
                // "Present", NO hover, not clickable. Covers both "already
                // marked present" (card body opens details) and "Upcoming /
                // not started" (nothing actionable yet). Raw button so the
                // muted look is exact (not the DS opacity-50 fade).
                <button
                    type="button"
                    disabled
                    className="w-full h-12 flex items-center justify-center gap-2 rounded-[10px] border-1 border-[#e4e7ec] bg-white text-[16px] font-semibold text-[#98a2b3] cursor-not-allowed"
                >
                    <CheckCircle className="w-5 h-5 text-[#98a2b3]" />
                    Present
                </button>
            )}
        </div>
...
export default function AttendeeDetailRoute() {
    return <Suspense fallback={null}><AttendeeDetailPage /></Suspense>;
}
...
function AttendeeDetailPage() {
        ...
        return (
            <div className="h-screen flex items-center justify-center">
                <div className="text-center">
                    ...
                    <button type="button" onClick={() => router.push("/attendee")}
    ...
    const TABS: { id: RosterTab; label: string; badge: string }[] = [
        ...
        { id: "waitlisted", label: "Waitlisted", badge: String(waitlistedRows.length) },
